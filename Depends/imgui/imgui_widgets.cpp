...
bool ImGui::ListBoxEx(const char* label, int* current_item, const char* const items[], int items_count, const ImVec2& size_arg,
    const ImVec4& cSelectColor)
{
    const bool value_changed = ListBoxEx(label, current_item, Items_ArrayGetter, (void*)items, items_count,
        size_arg, cSelectColor);
    return value_changed;
}

// This is merely a helper around BeginListBox(), EndListBox().
// Considering using those directly to submit custom data or store selection differently.
bool ImGui::ListBoxEx(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data,
    int items_count, const ImVec2& size_arg, const ImVec4& cSelectColor)
{
    ImGuiContext& g = *GImGui;

    if (!BeginListBox(label, size_arg))
        return false;
    ...
    while (clipper.Step())
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
            ...
            if (item_selected && cSelectColor.w)
                PushStyleColor(ImGuiCol_Text, cSelectColor);
